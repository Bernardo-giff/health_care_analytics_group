...
filtered_data <- filtered_data[!filtered_data$HOMPOP %in% c(98, 99), ]

# Visualize the distribution of HOMPOP with a histogram (discrete bins)
ggplot(filtered_data, aes(x = as.factor(HOMPOP), fill = as.factor(HOMPOP))) +
  geom_bar() +
...
ggplot(filtered_data, aes(y = HOMPOP)) +
  geom_boxplot(fill = "orange", color = "black") +
...
educ_type <- typeof(filtered_data$EDUC)
print(paste("Type of EDUC:", educ_type))

# Check for missing values in EDUC
missing_educ <- sum(is.na(filtered_data$EDUC))
print(paste("Number of missing EDUC values:", missing_educ))

# Show labels of EDUC
educ_labels <- attr(filtered_data$EDUC, "labels")
print("Labels for EDUC:")
print(educ_labels)
```
...
education_levels <- c(
  ...
  "10th grade", "11th grade", "12th grade", "1 year of college", "2 years",
  "3 years", "4 years", "5 years", "6 years", "7 years", "8 years",
  "Don't know", "No answer"
)

# Replace the numeric values with descriptions
filtered_data$EDUC <- factor(filtered_data$EDUC, levels = 0:20, labels = education_levels[1:21])
filtered_data$EDUC[filtered_data$EDUC == 98] <- "Don't know"
filtered_data$EDUC[filtered_data$EDUC == 99] <- "No answer"

# Visualize the distribution of EDUC with a bar plot
ggplot(filtered_data, aes(x = EDUC, fill = EDUC)) +
  geom_bar() +
...
table(filtered_data$SPWRKSTA)
```
Some visualisations
```{r}
# Visualize the distribution of SPWRKSTA with a bar plot
ggplot(filtered_data, aes(x = SPWRKSTA, fill = SPWRKSTA)) +
  geom_bar() +
...
spwrksta_probs <- prop.table(table(filtered_data$SPWRKSTA))
print(spwrksta_probs)

# Impute missing values randomly based on the distribution
set.seed(123)  # For reproducibility
filtered_data$SPWRKSTA[is.na(filtered_data$SPWRKSTA)] <- sample(
  names(spwrksta_probs), 
  sum(is.na(filtered_data$SPWRKSTA)), 
  replace = TRUE, 
  prob = spwrksta_probs
)

# Verify the imputation
missing_spwrksta_after <- sum(is.na(filtered_data$SPWRKSTA))
...
# Verify the conversion
print(levels(filtered_data$MARITAL))

# Remove rows with special codes in MARITAL
filtered_data <- filtered_data[filtered_data$MARITAL != "NA", ]

# Verify the changes
summary(filtered_data$MARITAL)
